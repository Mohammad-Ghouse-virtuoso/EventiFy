...
export default function ClosingCTA({ className = '', onSignUp }) {
  ...
  return (
    <section className={`w-full py-16 sm:py-20 ${className}`} aria-labelledby="closing-cta-title">
      <div className="bg-gradient-to-br from-pink-50 via-rose-50 to-amber-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-10 sm:gap-12 items-center py-10">
            {/* Copy */}
            <div>
              <h2 id="closing-cta-title" className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">
            ...
            <div className="relative animate-float-soft">
              <div
                className="w-full h-56 sm:h-64 md:h-72 rounded-2xl bg-white/70 border border-dashed border-gray-300 flex items-center justify-center overflow-hidden"
                aria-label="Playful doodle"
              >
                {doodle ? (
                  <img src={doodle} alt="Playful doodle" className="h-full w-full object-contain" />
                ) : (
                  <span className="text-gray-400 font-medium">Playful_doodle</span>
                )}
              </div>
              {/* Confetti-like accents */}
              <div className="absolute -top-3 -right-3 h-6 w-6 rounded-full bg-pink-300/70 blur-[1px]" aria-hidden="true" />
              <div className="absolute -bottom-4 -left-2 h-8 w-8 rounded-full bg-amber-300/70 blur-[1px]" aria-hidden="true" />
            </div>
          </div>
        </div>
      </div>
